...
#define ID_TEX_FLOWER 30
#define ID_TEX_JASON 40
...
void CSampleKeyHander::OnKeyDown(int KeyCode)
{
	...
	case DIK_SPACE:
		...
		if(jason->GetState()!=JASON_STATE_DIE&&jason->GetState()!=JASON_STATE_JUMP)
	...
	case DIK_A: // reset
		...
		jason->SetPosition(50.0f, 0.0f);
		jason->SetSpeed(0, 0);
	/*	for (int i = 0; i < objects.size(); i++) {
			if (dynamic_cast<CGoomba*>(objects[i])) {
				if (dynamic_cast<CGoomba*>(objects[i])->state == GOOMBA_STATE_DIE)
				{
					dynamic_cast<CGoomba*>(objects[i])->visible = true;
					dynamic_cast<CGoomba*>(objects[i])->state = GOOMBA_STATE_WALKING;
	...
	case DIK_X:
		if (jason->nx == -1 )
		{
			...
			float x1, y1;
			jason->GetPosition(x1, y1);
			bullet->SetPosition(x1 - BULLET_BBOX_WIDTH, y1 + JASON_BBOX_HEIGHT / 2 -  3);
			bullet->SetStartPositon(x1 + JASON_BBOX_WIDTH, y1 + JASON_BBOX_HEIGHT / 2 ,-1);
		...
		{			
				...
				float x1, y1;
				jason->GetPosition(x1, y1);
				bullet->SetPosition(x1 + JASON_BBOX_WIDTH, y1 + JASON_BBOX_HEIGHT / 2 -3);
				bullet->SetStartPositon(x1 + JASON_BBOX_WIDTH, y1 + JASON_BBOX_HEIGHT / 2 , 1);
				objects.push_back(bullet);
...
void LoadResources()
{
	...
	textures->Add(ID_TEX_JASON, L"textures\\Jason and Sophia.png", D3DCOLOR_XRGB(0, 57, 115));
	...
	LPDIRECT3DTEXTURE9 texFlower = textures->Get(ID_TEX_ENEMY);

	sprites->Add(40001, 143, 63, 175, 118, texFlower);
	sprites->Add(40002, 189, 62, 221, 118, texFlower);

	
	LPDIRECT3DTEXTURE9 texJason = textures->Get(ID_TEX_JASON);
	sprites->Add(50001, 258, 69, 282, 101, texJason);
	sprites->Add(50002, 233, 69, 257, 101, texJason);
	sprites->Add(50003, 208, 69, 232, 101, texJason);

	LPDIRECT3DTEXTURE9 texBullet = textures->Get(ID_TEX_JASON);
	sprites->Add(60001, 91, 20, 115, 28, texJason);
	sprites->Add(60002, 91, 29, 115, 28, texJason);
	...
	ani->Add(40001);				// Flower
	ani->Add(40002);
	animations->Add(801, ani);

	ani = new CAnimation(100);
	ani->Add(50001);
	ani->Add(50002);				//Jason walking
	ani->Add(50003);
	animations->Add(901, ani);

	ani = new CAnimation(100);
	ani->Add(50001);
	animations->Add(902, ani);     //Jason idle left



	ani = new CAnimation(100);
	ani->Add(60001);				//Bullet
	...
	CEnemy_Flower* flower = new CEnemy_Flower();
	flower->AddAnimation(801);
	flower->SetPosition(0 , 94);
	...
	jason = new CJason();
	jason->AddAnimation(902);
	jason->AddAnimation(902);
	jason->AddAnimation(901);
	jason->AddAnimation(901);
	jason->AddAnimation(902);
	jason->SetPosition(50.0f, 0);
